...
def A(id, sector, region, rev, s1, s2, s3, repl, target=0):
    return Asset(id=id, name=id, lat=0, lon=0, asset_type="x", replacement_value=repl,
        ...
        decarb_target_year=target)

# (rev, Scope1, Scope2, Scope3 tCO2, replacement/PP&E $, target). Sources in the write-up.
CO = [
    ...
    A("Caterpillar (ind.)", "manufacturing_general", "USA", 67e9, 1.4e6,  1.0e6, 400e6, 15e9, target=0),
...
print(f"{'Company':20s}{'S1+2 Mt':>8s}{'S3 Mt':>7s}{'PVcost':>8s}{'%ofRev/yr':>10s}"
      f"{'L1':>7s}{'L3':>7s}{'capex':>7s}{'impair':>8s}{'WACC':>6s}")
for r, a in zip(res, CO):
    ...
    print(f"{r.asset_id:20s}{(a.scope1_emissions_tco2+a.scope2_emissions_tco2)/1e6:>8.1f}"
          f"{a.scope3_emissions_tco2/1e6:>7.0f}{m(tot):>8s}{pct:>9.1f}%"
          f"{m(pv(lb['L1_carbon_opex'])):>7s}{m(pv(lb['L3_network_input_cost'])):>7s}"
          f"{m(pv(lb['L2_transition_capex'])):>7s}{m(sum(r.annual_impairment_usd.values())):>8s}"
