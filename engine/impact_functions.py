...
def _curve_payload(hazard: str, asset_type: str) -> tuple:
    key = _resolve_curve_key(asset_type)
    if hazard == "flood":
        curves = _flood()
        curve = curves.get(key, curves.get(asset_type, curves["_default"]))
        return tuple(curve["depth_m"]), tuple(curve["damage_fraction"]), key
    if hazard in ("wind", "cyclone"):
        curves = _wind()
        curve = curves.get(key, curves.get(asset_type, curves["_default"]))
        return tuple(curve["speed_ms"]), tuple(curve["damage_fraction"]), key
    if hazard == "wildfire":
        curves = _wildfire()
        curve = curves.get(key, curves.get(asset_type, curves["_default"]))
        return tuple(curve["flame_length_m"]), tuple(curve["damage_fraction"]), key
    if hazard == "heat":
        curves = _heat()
        cooling = curves["cooling_cost"]
        curve = cooling.get(key, cooling.get(asset_type, cooling["_default"]))
        return tuple(curve["temp_c"]), tuple(curve["damage_fraction"]), key
...
@lru_cache(maxsize=128)
def _curve_interpolator(hazard: str, asset_type: str):
    xs, ys, _ = _curve_payload(hazard, asset_type)
    if not xs:
        return None
    return PchipInterpolator(np.asarray(xs, dtype=float), np.asarray(ys, dtype=float))
...
def get_damage_fraction(hazard: str, asset_type: str, intensity: float) -> float:
    ...
    if intensity <= xs_arr[0]:
        return float(ys_arr[0])
    if intensity >= xs_arr[-1]:
        return float(ys_arr[-1])

    interp = _curve_interpolator(hazard, asset_type)
    return float(np.clip(interp(float(intensity)), 0.0, 1.0))

    return 0.0


def get_damage_fractions(hazard: str, asset_type: str, intensities: np.ndarray) -> np.ndarray:
    """Vectorized damage fractions for a hazard/asset curve."""
    ...
    if np.any(mid):
        interp = _curve_interpolator(hazard, asset_type)
        out[mid] = np.clip(interp(arr[mid]), 0.0, 1.0)
    return out
...
def get_damage_curve(hazard: str, asset_type: str, n_points: int = 100) -> tuple:
    ...
    xs_raw, _, _ = _curve_payload(hazard, asset_type)
    if not xs_raw:
        return np.array([]), np.array([])

    xs_arr = np.asarray(xs_raw, dtype=float)
    if hazard == "heat":
        x_min, x_max = xs_arr[0], xs_arr[-1]
    else:
        x_min, x_max = 0.0, xs_arr[-1]

    xs = np.linspace(x_min, x_max, n_points)
    ys = get_damage_fractions(hazard, asset_type, xs)
    return xs, ys
...
def get_curve_control_points(hazard: str, asset_type: str) -> tuple:
    ...
    xs, ys, key = _curve_payload(hazard, asset_type)
    if hazard == "flood":
        return np.array(xs, dtype=float), np.array(ys, dtype=float), "depth_m", key
    if hazard in ("wind", "cyclone"):
        return np.array(xs, dtype=float), np.array(ys, dtype=float), "speed_ms", key
    if hazard == "wildfire":
        return np.array(xs, dtype=float), np.array(ys, dtype=float), "flame_length_m", key
    if hazard == "heat":
        return np.array(xs, dtype=float), np.array(ys, dtype=float), "temp_c", key
    if hazard == "coastal_flood":
        return np.array(xs, dtype=float), np.array(ys, dtype=float), "depth_m", key
